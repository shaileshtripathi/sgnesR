...
RNG::ulong RNG::RNG::kn[128], RNG::RNG::ke[256];
double RNG::RNG::wn[128], RNG::RNG::fn[128], RNG::RNG::we[256], RNG::RNG::fe[256];

//________________________________________________________________________
// Initialize the static component of RNG

//________________________________________________________________________
// RNG::RNG::RNOR generates normal variates with rejection.
// nfix() generates variates after rejection in RNOR.
// Despite rejection, this method is faster than Box-Muller.

double RNG::RNG::nfix(slong h, ulong i)
{
...
// RNG::RNG::REXP generates exponential variates with rejection.
// efix() generates variates after rejection in REXP.
  
double RNG::RNG::efix(ulong j, ulong i)
{
  for (;;) {
    if (i == 0)
...
void RNG::RNG::zigset()
{
  // N.B. This should be safe even though the static variable
  // does not prevent multiple intialization

  static volatile bool inited = 0;
  if (inited)
    return;
  inited = 1;
...
double RNG::RNG::gamma(double shape, double scale)
{
  if (shape < 1.)
...
// NB. !!! Big fat warning !!!
	// The code below is likely to possess race conditions.
	// Since I don't use it, you can fix it yourself. -A
...
int RNG::RNG::binomial(double pp, int n)
{
...
void RNG::RNG::multinom(uint n, const std::vector<double>& probs, std::vector<uint>& samp)
{
  samp.resize(probs.size());
  RNG::multinom(n, &probs[0], (uint)probs.size(), &samp[0]);
}

void RNG::RNG::multinom(uint size, const double* probs, uint num_probs, uint* samp)
{
